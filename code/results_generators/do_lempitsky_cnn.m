function [ results ] = do_lempitsky_cnn( dict_name, sigma, NetN )
    ...
    [features, weights, crops] = loadFeatures('cells', 'CNN', images, sigma, dictionary, NetN);
    ...
    for d=1:length(distance_f)
        for N=Nvalues
            ...
            if ~exists(saving_name)
                splits = Lemptisky_split( N, images, 5, 100 );
                [results, bestC] = countingStars(splits, cropped_densities, features, weights, sigma, Cvalidation, 0, distance_f{d}, dictionary);
